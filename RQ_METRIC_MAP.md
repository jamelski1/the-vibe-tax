...
| M9 | LCB pass by framing × difficulty (**fixed scorer**) | Raw Data · LCB → `lcb_scored_stats.json` | | ● | ○ | | | Re-scored (capable models): terse 81.7 / casual 81.1 / detailed 81.1 / multilingual 84.1 — **flat**. Framing does not move correctness; difficulty does (easy 98.5 / med 87.3 / hard 60.5). |
...
| M12 | Pass rate by difficulty slice (easy / medium / hard) | Raw Data · LCB (difficulty slices) | | ○ | ● | | | Difficulty is what moves correctness: easy 98.5%, medium 87.3%, hard 60.5% (capable models) — a clean gradient. Framing is null within every slice (M10), so the "Goldilocks" story is about difficulty, not framing. |
